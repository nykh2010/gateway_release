...
function rstore_default_param(obj){
	$.post("/gateway/restore", obj, function (data) {
...
function DoUpdate() {
    ...
    var xsrf = document.getElementsByName("_xsrf")[0].value;
    var sharesult = $("#sha256").val();
    // obj._xsrf = $("#update_form").children("input[name='_xsrf']").val();
    var file_obj = document.getElementById("put_file").files[0];
    obj.append("_xsrf", xsrf);
    obj.append("put_file", file_obj);
    obj.append("sha256", sharesult);
    // obj.put_file = $("#put_file")[0];
    ...
    try
    {
    	...
    	// $.post("/gateway/upgrade",{data:obj},function(data){
     //        if (data.status == "success") {
     //            console.log("post success");
     //            show_message("success");
     //            return;
...
function update_iframe_check() {
	var xsrf_tmp = $("#dialog_frm").children("input[name='_xsrf']").val();
	$.post("/gateway/upgrade",{_xsrf:xsrf_tmp},function(data){
		if (data.status == "success") {
			show_message('success');
			return;
		}
...
function user_password_set() {
    if (check_input("user_pwd_frm")) {
        ...
        */
        ...
        var xsrf_tmp = $("#user_pwd_frm").children("input[name='_xsrf']").val();
        obj.opwd = opwd;
        obj.pwd1 = pwd1;
        obj.pwd2 = pwd2;
        obj._xsrf = xsrf_tmp;
        $.post("/auth/update", obj, function (data) {
            if (data.status == "success") {
                show_message("success");
                loginOut();
            }
...
function init_system_time() {
    ...
    //     var data = dataDeal(data);
        ...
        var date_time = strtime.split("&nbsp");
...
function system_time_submit() {
    ...
    show_message("save");
    var obj = {};
    obj = igd.ui.form.collect("system_time_frm");
    var xsrf_tmp = $("#system_time_frm").children("input[name='_xsrf']").val();
    obj._xsrf = xsrf_tmp;
    $.post("/gateway/update/time", obj, function (data) {
        if (data.status == "success")
            show_message("success");
        else
            show_message("error", igd.make_err_msg(data));
...
//    });
	var obj = {}
	obj._xsrf = $("#timed_reboot_form").children("input[name='_xsrf']").val();
    $.post("/gateway/restart", obj);//by lwj 无线在某些设备中会马上断网，接收不到返回信息
...
function set_miscreboot() {
    ...
    var xsrf_tmp = $("#timed_reboot_form").children("input[name='_xsrf']").val();
    obj._xsrf = xsrf_tmp;
    $.post("/gateway/restart", obj, function (data) {
        if (data == "success")
            show_message("success");
        else
            show_message("error", data.err_msg);
