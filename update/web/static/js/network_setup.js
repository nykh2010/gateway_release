...
function set_wan_link_work_mode(index, isMobile) {
    ...
    var loadCallBackFn = function (ret) {
        $(document).ready(function(){
            ...
            if(igd.global_param.is_1000M===true){
                $('#wan1_select').append("<option value='1000f'>1000M全双工</option>")
            }
            if (connecttype)
...
function submit_wan_config(index,callback) {
    ...
    if (ret) {
        ...
        var connecttype_to_res;
        obj.uiname = $("#common_uiname").val();
        obj.connect_type = $("#common_connect_type").val();
        if(obj.connect_type==="PPPOE"){
            connecttype_to_res=1;
        }else if(obj.connect_type==='DHCP'){
            connecttype_to_res=2;
        }else if(obj.connect_type==='STATIC'){
            connecttype_to_res=3;
        }
        //校验密码
            ...
            if (data == "SUCCESS") {
                setTimeout(function(){
                    ...
                    if ($("#wan1_select").val() == conf_speed_value) {
                       ...
                       $.when(wan_setup_s(connecttype_to_res)).done(function(){show_message('success');});
                    }
                    else {
                        set_interface_mode();
                    }
